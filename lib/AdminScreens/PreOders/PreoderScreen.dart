...
class _PreoderScreenState extends State<PreoderScreen> {
  ...
  String tokendata = "";
  List<dynamic> PreoderScreendata = []; //ประกาศตัวแปร อาร์เรย์ ไว้
  int totalResults = 0;
  int page = 1;
  int pageSize = 10;
  ...
  void _onRefresh() async {
    ...
    setState(() {
      PreoderScreendata.clear();
      page = 1;
    });
    _getPreoderScreenory(); //ทุกครั้งที่ทำการรีเฟรช จะดึงข้อมูลใหม่
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          PreoderScreendata.addAll(depdata['data']['data']);
          isLoading = false;
  ...
  _PreoderScreenoryType() async {
    ...
    var url = Uri.parse(pathAPI + 'api/PreoderScreenory_type');
    var response = await http.get(
      url,
      headers: {
        'Content-Type': 'application/json',
    ...
    if (response.statusCode == 200) {
      final Map<String, dynamic> dataPreoderScreen =
          convert.jsonDecode(response.body);
      if (dataPreoderScreen['code'] == 200) {
        //print(dataPreoderScreen['message']);
        setState(() {
          dropdownValue = dataPreoderScreen['data'];
        });
  ...
  _createPreoderScreenory(Map<String, dynamic> values) async {
    ...
    var url = Uri.parse(pathAPI + 'api/create_PreoderScreenory');
    ...
    if (response.statusCode == 201) {
      final Map<String, dynamic> PreoderScreendata =
          convert.jsonDecode(response.body);
      print(PreoderScreendata);
      if (PreoderScreendata['code'] == 201) {
        print(PreoderScreendata['message']);
        //MyNavigator.goToPreoderScreen(context);
        // Navigator.push(
        ...
        //   context: context,
        //   builder: (context) => alertPreoderScreen(
        //     PreoderScreendata['message'],
        //     picSuccess,
        //     context,
        //   ),
        // );
